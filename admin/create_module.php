...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (move_uploaded_file($file_upload['tmp_name'], $file_upload_path)) {
        ...
        $sql = "INSERT INTO modules (id, nama_praktikum, nama_pertemuan, deskripsi, file_upload, link, created_at, file_name) 
                VALUES (NULL, '$nama_praktikum', '$nama_pertemuan', '$deskripsi', '$file_upload_name', '$link', current_timestamp(), '$file_name')";
...
<body>
            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form id="add-module-form" method="post" action="" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="nama_praktikum" aria-label="Default select example">
                                    ...
                                    while ($row = mysqli_fetch_assoc($data_courses)) {
                                        echo '<option value="' . htmlspecialchars($row['nama_praktikum']) . '">' . htmlspecialchars($row['nama_praktikum']) . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="nama_pertemuan" aria-label="Default select example">
                                    ...
                                    while ($row = mysqli_fetch_assoc($data_meetings)) {
                                        echo '<option value="' . htmlspecialchars($row['nama_pertemuan']) . '">' . htmlspecialchars($row['nama_pertemuan']) . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                            ...
                            <div class="text-end">
                                <button type="submit" class="btn btn-primary" id="save-btn">Add Module</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    <script>
        document.getElementById('add-module-form').addEventListener('submit', function(event) {
                ...
                success: function(response) {
                    ...
                    if (res.status === 'success') {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error!',
                            text: 'Failed to add module: ' + res.message,
                        });
                    }
                },
